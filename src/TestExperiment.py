...
class TestExperiment(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sdt_zero = SignalDetection(0, 0, 0, 0)
        self.sdt_half = SignalDetection(100, 0, 0, 0)
        self.sdt_full = SignalDetection(100, 0, 100, 0)
        self.exp = Experiment()
    ...
    def test_auc_valid_two_conditions(self):
    ...
    def test_auc_valid_three_conditions(self):
    ...
    def test_compute_auc_known_conditions(self):
        """Test specific AUC values for known cases."""
        # Case with (0,0) and (0,1) should give AUC = 0.0
        exp = Experiment()
        exp.add_condition(self.sdt_zero) # (0,0)
        exp.add_condition(self.sdt_half) # (0,1)
        auc = exp.compute_auc()
        self.assertAlmostEqual(auc, 0.0)

    def test_compute_auc_known_conditions2(self):
        """Test specific AUC values for known cases."""
        # Case with (0,0) and (1,1) should give AUC = 0.5
        exp = Experiment()
        exp.add_condition(self.sdt_zero) # (0,0)
        exp.add_condition(self.sdt_full) # (1,1)
        auc = exp.compute_auc()
        self.assertAlmostEqual(auc, 0.5)

    def test_compute_auc_known_conditions3(self):
        """Test specific AUC values for known cases."""
